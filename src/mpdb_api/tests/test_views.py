...
from mpdb_api.models import Movie, Comment, Watchlist, Favorite, UserRating
...
class TestCommentAPI(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        movie = Movie.objects.create(
            ...
            production="Buena Vista", website="N/A",
            created=str(date.today()))
        Comment.objects.create(user=user, movie=movie,
                               body='Great Movie!')
        Comment.objects.create(user=user, movie=movie,
                               body='Great Movie!')
...
class TestWatchlistAPI(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        Watchlist.objects.create(user=user, movie=movie,
                                 added=True)
        Watchlist.objects.create(user=user, movie=movie2,
                                 added=True)
...
class TestFavoriteAPI(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        Favorite.objects.create(user=user, movie=movie,
                                added=True)
        Favorite.objects.create(user=user, movie=movie2,
                                added=True)
...
class TestUserRatingAPI(APITestCase):
    ...
    def test_create_user_rating_out_of_scope(self):
        self.assertTrue(self.client.login(username='Test',
                                          password='TestPw4foranewuser'))
        rating_body = {"movie": "http://127.0.0.1:8000/api/v1/movies/1/",
                       "rate": 6}
    ...
    def test_get_retrive_user_rating(self):
    ...
    def test_update_user_rating(self):
        ...
        self.assertEqual(user_rating_list_response.data['count'], 2,
                         msg='A list should include 2 movies.')
...
class TestCustomUserAPI(APITestCase):
    ...
    #     response = self.client('/api/v1/users/')
    #     self.assertTrue(response)
    #     self.assertEqual(response.status_code, status.HTTP_200_OK)
